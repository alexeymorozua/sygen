...
- persisted in `~/.sygen/tasks.json`
- task folders in `~/.sygen/workspace/tasks/<task_id>/`
...
- forum-topic tasks route back to the originating topic via `thread_id` / `SYGEN_TOPIC_ID`
