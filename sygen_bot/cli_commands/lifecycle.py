...
def _re_exec_bot() -> NoReturn:
    ...
    Spawns a new Python process running ``sygen_bot`` and exits the current one.
    Under a service manager the caller should ``sys.exit(EXIT_RESTART)`` instead.
    """
    subprocess.Popen([sys.executable, "-m", "sygen_bot"])
    sys.exit(0)
...
def _stop_service_if_running() -> None:
    ...
    with contextlib.suppress(Exception):
        from sygen_bot.infra.service import is_service_installed, is_service_running, stop_service
...
def stop_bot() -> None:
    ...
    """
    from sygen_bot.infra.pidlock import _is_process_alive, _kill_and_wait
...
def start_bot(verbose: bool = False) -> None:
    ...
    from sygen_bot.__main__ import load_config, run_telegram
    from sygen_bot.logging_config import setup_logging
