...
- `~/.ductor/agents/<name>/workspace/output_to_user/`
...
Check `~/.ductor/workspace/tasks/` for task directories.
...
Also check sub-agent task directories: `~/.ductor/agents/*/workspace/tasks/`

### 3. Clean orphaned CLI session files

Look for `.jsonl` files in `~/.ductor/sessions/` (or wherever session logs are stored).
Read `~/.ductor/sessions.json` to find referenced session files.
