...
from sygen_bot.commands import BOT_COMMANDS as _COMMAND_DEFS
from sygen_bot.commands import MULTIAGENT_SUB_COMMANDS as _MA_SUB_DEFS
...
from sygen_bot.messenger.telegram.chat_tracker import ChatRecord, ChatTracker
from sygen_bot.messenger.telegram.file_browser import (
    file_browser_start,
...
from sygen_bot.messenger.telegram.handlers import (
    handle_abort,
    handle_abort_all,
    handle_command,
    handle_interrupt,
...
from sygen_bot.messenger.telegram.media import (
    has_media,
...
from sygen_bot.messenger.telegram.middleware import (
    MQ_PREFIX,
    AuthMiddleware,
...
from sygen_bot.messenger.telegram.topic import (
    TopicNameCache,
    get_session_key,
    get_thread_id,
)
from sygen_bot.messenger.telegram.typing import TypingContext as _TypingContext
from sygen_bot.messenger.telegram.welcome import (
...
class TelegramBot:
    ...
    def __init__(
        ...
        *,
        ...
        from sygen_bot.messenger.telegram.transport import TelegramTransport
    ...
    async def _on_startup(self) -> None:
        from sygen_bot.messenger.telegram.startup import run_startup
    ...
    def _register_handlers(self) -> None:
        ...
        base_cmds = ["status", "memory", "model", "cron", "logs", "diagnose", "upgrade"]
    ...
    async def _send_join_notification(self, chat_id: int) -> None:
        ...
        if not text:
            return
        from sygen_bot.messenger.telegram.sender import _send_text_chunks
    ...
    async def _on_forum_topic_created(self, message: Message) -> None:
        ...
        from sygen_bot.messenger.telegram.topic import get_topic_name_from_message
    ...
    async def _on_session(self, message: Message) -> None:
        ...
        try:
            if session_followup:
                ...
                from sygen_bot.orchestrator.core import NamedSessionRequest
    ...
    async def _on_restart(self, message: Message) -> None:
        if self._config.group_mention_only and not self._is_addressed(message):
            return
        from sygen_bot.infra.restart import write_restart_sentinel
    ...
    async def _route_special_callback(
        ...
        from sygen_bot.orchestrator.selectors.model_selector import is_model_selector_callback

        if is_model_selector_callback(data):
            await self._handle_model_selector(key, message_id, data)
            return True

        from sygen_bot.orchestrator.selectors.cron_selector import is_cron_selector_callback
    ...
    async def _route_prefix_callback(
        ...
        from sygen_bot.orchestrator.selectors.session_selector import is_session_selector_callback
        from sygen_bot.orchestrator.selectors.task_selector import is_task_selector_callback
    ...
    async def _handle_model_selector(self, key: SessionKey, message_id: int, data: str) -> None:
        ...
        from sygen_bot.orchestrator.selectors.model_selector import handle_model_callback
    ...
    async def _handle_cron_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from sygen_bot.orchestrator.selectors.cron_selector import handle_cron_callback
    ...
    async def _handle_session_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from sygen_bot.orchestrator.selectors.session_selector import handle_session_callback
    ...
    async def _handle_task_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from sygen_bot.orchestrator.selectors.task_selector import handle_task_callback
    ...
    async def _handle_ns_callback(
        ...
        async with self._sequential.get_lock(key.lock_key):
            if self._config.streaming.enabled:
                from sygen_bot.orchestrator.flows import named_session_streaming

                result = await named_session_streaming(self._orch, key, session_name, label)
            else:
                from sygen_bot.orchestrator.flows import named_session_flow
    ...
    async def _set_reaction(self, message: Message, emoji: str) -> None:
        """Set an emoji reaction on *message*. Graceful degradation on failure."""
        try:
    ...
    async def on_async_interagent_result(self, result: AsyncInterAgentResult) -> None:
        ...
        from sygen_bot.bus.adapters import from_interagent_result
    ...
    async def on_task_result(self, result: TaskResult) -> None:
        ...
        from sygen_bot.bus.adapters import from_task_result
    ...
    async def on_task_question(
        ...
        from sygen_bot.bus.adapters import from_task_question
    ...
    async def _handle_webhook_wake(self, chat_id: int, prompt: str) -> str | None:
        ...
        from sygen_bot.bus.envelope import LockMode
        ...
        from sygen_bot.bus.adapters import from_webhook_wake
    ...
    async def _handle_upgrade_callback(
        ...
        from sygen_bot.messenger.telegram.upgrade_handler import handle_upgrade_callback
