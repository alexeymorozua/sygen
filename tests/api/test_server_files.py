...
pytest.importorskip("nacl", reason="PyNaCl not installed (optional: pip install sygen[api])")

from aiohttp import FormData, web

from sygen_bot.api.server import _parse_file_refs
...
class TestParseFileRefs:
    ...
    def test_windows_file_ref_is_normalized(self) -> None:
        with patch("sygen_bot.files.tags.is_windows", return_value=True):
...
class TestFileUpload:
    ...
    def test_uploaded_file_exists_on_disk(self, tmp_path: Path) -> None:
        ...
        from sygen_bot.files.storage import prepare_destination
