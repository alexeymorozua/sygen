...
from sygen_bot.cli.param_resolver import TaskOverrides, resolve_cli_config
from sygen_bot.config import AgentConfig
from sygen_bot.cron.execution import build_cmd
...
def mock_paths(tmp_path: Path) -> SygenPaths:
    """Create mock SygenPaths for testing."""
    home = tmp_path / "sygen_home"
    fw = tmp_path / "fw"
    paths = SygenPaths(
        sygen_home=home,
...
async def test_cron_task_model_override(
    ...
    # Build command
    with patch("sygen_bot.cron.execution.which", return_value="/usr/bin/claude"):
...
async def test_cron_task_cli_parameters(
    ...
    # Build command
    with patch("sygen_bot.cron.execution.which", return_value="/usr/bin/codex"):
        result = build_cmd(exec_config, "Test prompt")
