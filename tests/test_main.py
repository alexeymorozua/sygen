...
_STATUS = "sygen_bot.cli_commands.status"
_SERVICE = "sygen_bot.cli_commands.service"
...
class TestRunTelegram:
    ...
    async def test_runs_bot_with_valid_config(self, tmp_path: Path) -> None:
        ...
        with (
            patch("sygen_bot.__main__.resolve_paths"),
            patch("sygen_bot.infra.pidlock.acquire_lock"),
            patch("sygen_bot.infra.pidlock.release_lock"),
            patch(
                "sygen_bot.multiagent.supervisor.AgentSupervisor",
...
class TestIsConfiguredExtended:
    def test_unconfigured_with_placeholder_token(self, tmp_path: Path) -> None:
        ...
        with patch("sygen_bot.__main__.resolve_paths", return_value=paths):
            assert _is_configured() is False
...
class TestStopBot:
    def test_stop_kills_running_process(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("sygen_bot.infra.pidlock._is_process_alive", return_value=True),
            patch("sygen_bot.infra.pidlock._kill_and_wait") as mock_kill,
        ):
            stop_bot()
...
class TestUpgradeCli:
    ...
    def test_upgrade_rejects_dev_mode(self) -> None:
        ...
        with (
            patch("sygen_bot.infra.install.detect_install_mode", return_value="dev"),
...
class TestReExecBot:
    def test_re_exec_uses_popen_on_posix(self) -> None:
        from sygen_bot.cli_commands.lifecycle import _re_exec_bot
        ...
        with (
            ...
            _re_exec_bot()
        mock_popen.assert_called_once_with([sys.executable, "-m", "sygen_bot"])
...
class TestStartBotRestart:
    ...
    def test_exit42_with_supervisor_exits(self) -> None:
        from sygen_bot.cli_commands.lifecycle import start_bot
...
class TestCountLogErrors:
    def test_counts_errors_in_log(self, tmp_path: Path) -> None:
        from sygen_bot.cli_commands.status import count_log_errors
...
class TestSetupCommand:
    def test_setup_starts_bot_when_service_not_installed(self) -> None:
        from sygen_bot.__main__ import _cmd_setup

        with (
            patch("sygen_bot.__main__._stop_bot"),
            patch("sygen_bot.__main__.resolve_paths"),
            patch("sygen_bot.__main__._is_configured", return_value=False),
            patch("sygen_bot.cli.init_wizard.run_onboarding", return_value=False),
            patch("sygen_bot.__main__._start_bot") as mock_start,
        ):
            _cmd_setup(False)
        mock_start.assert_called_once_with(False)
...
class TestMainHelpers:
    def test_parse_service_subcommand_ignores_flags(self) -> None:
        from sygen_bot.cli_commands.service import _parse_service_subcommand
    ...
    def test_cmd_service_without_subcommand_prints_help(self) -> None:
        from sygen_bot.cli_commands.service import cmd_service
    ...
    def test_cmd_service_install_dispatches_backend(self) -> None:
        ...
        with patch("sygen_bot.infra.service.install_service") as mock_install:
    ...
    def test_cmd_service_status_dispatches_backend(self) -> None:
        ...
        with patch("sygen_bot.infra.service.print_service_status") as mock_status:
            cmd_service(["service", "status"])
        mock_status.assert_called_once()

    def test_print_usage_calls_status_when_configured(self) -> None:
        from sygen_bot.cli_commands.status import print_usage
    ...
    def test_print_usage_shows_not_configured_panel(self) -> None:
        ...
        with patch("sygen_bot.__main__._is_configured", return_value=False):
            print_usage()

    def test_cmd_status_prints_not_configured(self) -> None:
        from sygen_bot.__main__ import _cmd_status

        with patch("sygen_bot.__main__._is_configured", return_value=False):
            _cmd_status()

    def test_cmd_restart_stops_and_reexecs(self) -> None:
        from sygen_bot.cli_commands.lifecycle import cmd_restart
    ...
    def test_default_action_configured_starts(self) -> None:
        from sygen_bot.__main__ import _default_action
