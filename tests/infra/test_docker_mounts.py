...
class TestDockerManagerMounts:
    ...
    async def test_single_mount_in_run_cmd(self, tmp_path: Path, docker_paths: DuctorPaths) -> None:
        ...
        with (
            patch("sygen_bot.infra.docker.which", return_value="/usr/bin/docker"),
...
class TestDockerMountCLI:
    ...
    def test_mount_adds_path(self, tmp_path: Path) -> None:
        from sygen_bot.cli_commands.docker import docker_mount
        ...
        with patch("sygen_bot.cli_commands.docker.resolve_paths", return_value=paths_mock):
    ...
    def test_unmount_removes_path(self, tmp_path: Path) -> None:
        from sygen_bot.cli_commands.docker import docker_unmount
    ...
    def test_mounts_list_empty(self, tmp_path: Path) -> None:
        from sygen_bot.cli_commands.docker import docker_list_mounts
