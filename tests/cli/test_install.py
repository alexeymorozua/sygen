...
from sygen_bot.cli_commands.install import (
    _install_extra,
    _is_installed,
    cmd_install,
    print_install_help,
...
class TestCmdInstall:
    ...
    def test_no_args_shows_help(self) -> None:
        with patch("sygen_bot.cli_commands.install.print_install_help") as mock_help:
            cmd_install([])
    ...
    def test_valid_extra_calls_install(self) -> None:
        with patch("sygen_bot.cli_commands.install._install_extra") as mock_install:
...
class TestInstallExtra:
    ...
    def test_already_installed_shows_message(self) -> None:
        with patch("sygen_bot.cli_commands.install._is_installed", return_value=True):
            console = MagicMock()
            with patch("sygen_bot.cli_commands.install.Console", return_value=console):
    ...
    def test_not_installed_runs_subprocess_pip(self) -> None:
        with (
            patch("sygen_bot.cli_commands.install._is_installed", return_value=False),
            patch(
                "sygen_bot.cli_commands.install.detect_install_mode",
                return_value="pip",
            ),
            patch("sygen_bot.cli_commands.install.subprocess.run") as mock_run,
            patch("sygen_bot.cli_commands.install.Console", return_value=MagicMock()),
        ):
    ...
    def test_failed_install_shows_error(self) -> None:
        with (
            ...
            patch("sygen_bot.cli_commands.install.subprocess.run") as mock_run,
            patch("sygen_bot.cli_commands.install.Console", return_value=MagicMock()) as console,
        ):
