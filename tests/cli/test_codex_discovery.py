...
from sygen_bot.cli.codex_discovery import CodexModelInfo, discover_codex_models
...
async def test_discover_models_parses_response() -> None:
    proc = _mock_process()
    with (
        patch("sygen_bot.cli.codex_discovery.which", return_value="/usr/bin/codex"),
...
async def test_discover_models_codex_not_installed() -> None:
    with patch("sygen_bot.cli.codex_discovery.which", return_value=None):
        models = await discover_codex_models()
    assert models == []
