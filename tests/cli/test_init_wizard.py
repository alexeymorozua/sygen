...
def test_write_config_ignores_corrupt_existing_json(tmp_path: Path) -> None:
    ...
    with (
        patch("sygen_bot.cli.init_wizard.resolve_paths", return_value=paths),
        patch("sygen_bot.cli.init_wizard.init_workspace"),
    ):
        out = _write_config(
            _WizardConfig(
                transport="telegram",
...
def test_run_onboarding_returns_false_when_service_install_fails(tmp_path: Path) -> None:
    ...
    with (
        patch("sygen_bot.cli.init_wizard._show_banner"),
        patch("sygen_bot.cli.init_wizard._check_clis"),
        patch("sygen_bot.cli.init_wizard._show_disclaimer"),
        patch("sygen_bot.cli.init_wizard._ask_transport", return_value="telegram"),
        patch("sygen_bot.cli.init_wizard._ask_telegram_token", return_value="token"),
        patch("sygen_bot.cli.init_wizard._ask_user_id", return_value=[1]),
        patch("sygen_bot.cli.init_wizard._ask_docker", return_value=False),
        patch("sygen_bot.cli.init_wizard._ask_timezone", return_value="UTC"),
        patch("sygen_bot.cli.init_wizard._write_config", return_value=paths.config_path),
        patch("sygen_bot.cli.init_wizard.resolve_paths", return_value=paths),
        patch("sygen_bot.cli.init_wizard._offer_service_install", return_value=True),
        patch("sygen_bot.infra.service.install_service", return_value=False),
    ):
...
def test_run_onboarding_returns_true_when_service_install_succeeds(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("sygen_bot.cli.init_wizard.resolve_paths", return_value=paths),
        patch("sygen_bot.cli.init_wizard._offer_service_install", return_value=True),
        patch("sygen_bot.infra.service.install_service", return_value=True),
    ):
