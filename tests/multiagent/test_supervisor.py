...
from sygen_bot.multiagent.supervisor import (
...
def main_config(tmp_path: Path) -> AgentConfig:
    """Create a main config with tmp_path as sygen_home."""
    return AgentConfig(
        sygen_home=str(tmp_path),
...
class TestStartupFailures:
    ...
    async def test_internal_api_start_failure_is_propagated(
        ...
        with (
            ...
            patch("sygen_bot.multiagent.supervisor.AgentStack.create", new_callable=AsyncMock),
...
class TestOnAgentsChanged:
    ...
    async def test_restarts_on_token_change(
        ...
        old_config = AgentConfig(
            telegram_token="old-token", sygen_home=str(tmp_path / "agents/sub1")
        )
...
class TestStartSubAgent:
    ...
    async def test_handles_creation_failure(self, supervisor: AgentSupervisor) -> None:
        ...
        with patch(
            "sygen_bot.multiagent.supervisor.AgentStack.create",
