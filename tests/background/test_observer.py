...
from sygen_bot.background.models import BackgroundResult, BackgroundSubmit
from sygen_bot.background.observer import MAX_TASKS_PER_CHAT, BackgroundObserver
from sygen_bot.cli.param_resolver import TaskExecutionConfig
from sygen_bot.cron.execution import OneShotExecutionResult
...
def _make_observer(paths: SygenPaths, timeout: float = 300.0) -> BackgroundObserver:
...
@pytest.fixture
async def observer(paths: SygenPaths) -> AsyncIterator[BackgroundObserver]:
...
class TestSubmit:
    async def test_returns_task_id(self, observer: BackgroundObserver) -> None:
        ...
        with patch(
            "sygen_bot.background.observer.run_oneshot_task",
...
class TestExecution:
    async def test_success_delivers_result(self, observer: BackgroundObserver) -> None:
        ...
        with patch("sygen_bot.background.observer.run_oneshot_task", return_value=result):
