...
from sygen_bot.cli.codex_discovery import CodexModelInfo
...
async def test_orchestrator_starts_cache_observer(mock_codex_cache: CodexModelCache) -> None:
    ...
    with (
        patch("sygen_bot.orchestrator.observers.CodexCacheObserver", return_value=mock_observer),
        patch("sygen_bot.orchestrator.lifecycle.resolve_paths"),
        patch("sygen_bot.orchestrator.lifecycle.inject_runtime_environment"),
...
async def test_orchestrator_passes_cache_to_observers(
    ...
    with (
        patch(
            "sygen_bot.orchestrator.observers.CodexCacheObserver", return_value=mock_cache_observer
        ),
        patch("sygen_bot.orchestrator.observers.CronObserver", mock_cron_class),
        patch("sygen_bot.orchestrator.observers.WebhookObserver", mock_webhook_class),
        patch("sygen_bot.orchestrator.lifecycle.resolve_paths"),
        patch("sygen_bot.orchestrator.lifecycle.inject_runtime_environment"),
        patch("sygen_bot.cli.auth.check_all_auth", return_value={}),
    ):
